...
router.post('/', auth, (req, res) => {
  try {
    const { bus_id, seat_ids, seat_numbers, total_fare } = req.body
    const ids = seat_ids || seat_numbers

    if (!bus_id || !ids || !Array.isArray(ids) || ids.length === 0) {
    ...
    const seats = queryAll(`SELECT id, seat_number, is_booked, bus_id FROM seats WHERE id IN (${placeholders})`, ids)
    ...
    const expectedFare = ids.length * bus.fare
    ...
    execute(
      ...
      [req.user.id, bus_id, JSON.stringify(ids), total_fare, now]
    )
